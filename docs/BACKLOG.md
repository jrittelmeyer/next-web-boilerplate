...
| B1 | On-ramp | **`init-app --slim` leftover-pointer tidy #2** — the post-slim report flags two live leftovers U1's list doesn't cover: `docs/MAINTENANCE.md:71` (BACKLOG link) + `:114` (archive link); extend the tidy list or reword the lines | scripts/init-app.mjs · [GETTING_STARTED.md](GETTING_STARTED.md#remove-what-you-dont-need) | Surfaced by the project-adopt live trial 2026-07-19 (linkding scaffold). Small; ride-along-able. |
| B3 | Docs / UI | **UI.md token-sheet adoption recipe** — a worked "adopt an existing brand/token sheet onto the shadcn/Tailwind theme" recipe (map an extracted palette/type/space/breakpoint sheet onto `@repo/tailwind-config` tokens, both themes) | [context/UI.md](context/UI.md) | Surfaced by the project-adopt live trial: every adoption's keep-theirs design system needs this path; the linkding token sheet is the worked example shape. |
