...
| B3 | Docs / marketing | **Visual surface — README screenshot tour** — a short screenshot tour (landing · dashboard · `/account` · dark mode) in the README / FEATURES, captured from a real local run (live-verify style) | README.md · [docs/FEATURES.md](FEATURES.md) | README carries 2 badges and 0 screenshots; the first scroll decides template adoption. **The hosted-gallery half (b) shipped 2026-07-20** — Storybook on GitHub Pages (strikethrough row below); this is the remaining screenshot half (a). Verify: shots from a real running app; images resolve on GitHub. Plan → sign-off. |
