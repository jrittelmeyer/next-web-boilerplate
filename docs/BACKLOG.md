...
| B1 | On-ramp / kit | **Non-technical self-service adoption wrapper for `/project-adopt`** — repo half: a `--slim`-surviving `docs/ADOPTING_AN_EXISTING_APP.md` runbook (prereqs → acquire template → drop code into `intake/source/` → `/project-adopt` kickoff → plain-English question-round + informed sign-off + "your current site keeps running"/cutover + a "did I lose a feature?" parity self-check + new-DX onboarding) · `scripts/adopt-preflight.mjs` (stdlib-only, runs **before** `pnpm install`: Node 24 / pnpm / Docker / git / ports 3000+3100 / `.env` seeded → plain PASS/FAIL + fix link each) · README adoption pointer · GETTING_STARTED link · materialize `intake/source/` with a tracked `.gitkeep`. Kit half: add an **"agentic layer" survey line** to project-adopt §2 + a **keep/port/replace disposition bucket** to §3 + a plain-language pre-sign-off summary nudge (§5/§8), so an incoming project's own `.claude/` (its skills/hooks/CLAUDE.md/memory) isn't silently dropped | [GETTING_STARTED.md](GETTING_STARTED.md#starting-from-an-existing-app-run-project-adopt) · [intake/README.md](../intake/README.md) | Motivating case: a non-technical owner brings a brownfield app (built with Claude Code, unknown stack, its own `.claude/`) onto the template with **no feature loss** + a DX upgrade, self-served with near-zero direction. **Gap verified 2026-07-24** (multi-agent survey): `/project-adopt` is the complete *engine* (consume → parity contract → theirs-vs-template disposition → PRODUCT/MIGRATION → banded port backlog) but agent-altitude only — no non-technical runbook, no toolchain preflight, no plain-English gate/cutover/verify layer, no incoming-`.claude/` reconcile. **Two owner decisions pending:** (D) whether an optional `scripts/adopt-inventory.mjs` mechanical-seed script earns its keep (must only *seed* the §2 survey, never replace §3 judgment); (E) land the incoming-`.claude/` survey as a **kit-upstream change now** vs **interim runbook guidance + a filed upstream row**. Kit half edits an ai-dev-kit clone → reinstall (`--check` clean), never the installed copies. Runbook + DX doc must live in a `--slim`-surviving path (verify against `init-app.mjs`, not `docs/plain-english-guide/`). Ready-to-paste build-prompt drafted. Plan → sign-off before building. |
