...
| B1 | On-ramp / release | **Tagged releases + a template-update path for derived apps** — (a) cut annotated git tags + GitHub Releases and tag on milestones thereafter (the CHANGELOG has a `[1.0.0]` header but **git has no tag and there is no GitHub release** — start by tagging the launch commit v1.0.0 and drafting the release from the existing notes; decide the version for the current tip at sign-off); (b) a GETTING_STARTED "Staying current with the template" recipe — `git remote add template …` → fetch → merge/cherry-pick, with honest conflict-zone notes (lockfile, `.env.example`, docs `/project-init` regenerates) | [GETTING_STARTED.md](GETTING_STARTED.md) · CHANGELOG.md | `degit` is a one-way copy — an adopter can't pull later template improvements; this is the one structural gap in the "maintained template" claim. Verify: tag + draft release; dry-run the update-merge on a degit copy. Pairs with the derived-product driver (Watch). Plan → sign-off. |
| B3 | CI / ops | **Scheduled CI heartbeat on `main`** — add a weekly `schedule:` (cron) trigger + `workflow_dispatch` to `ci.yml` so the full pipeline (incl. e2e / docker-image) runs between merges | [.github/workflows/ci.yml](../.github/workflows/ci.yml) · [context/DEPLOYMENT.md](context/DEPLOYMENT.md) | In maintenance mode a push is the only thing exercising CI, so "green" can rot silently against world-drift (base-image pulls, registry behavior, provider-facing flows). CodeQL already has a cron; `ci.yml` runs only on push/PR. Renovate activity keeps the cron clear of GitHub's 60-day scheduled-workflow auto-disable. Verify: a `workflow_dispatch` run green, then a scheduled run green. Plan → sign-off. |
| B3 | Docs / marketing | **Visual surface — README screenshots + a hosted component gallery** — (a) a short screenshot tour (landing · dashboard · `/account` · dark mode) in the README / FEATURES; (b) publish the already-built static Storybook (`pnpm --filter @repo/ui build-storybook` → `storybook-static`, gitignored) to GitHub Pages via a deploy workflow | README.md · [docs/FEATURES.md](FEATURES.md) · new `.github/workflows/pages.yml` | README carries 2 badges and 0 screenshots; the first scroll decides template adoption. The Storybook export already exists — Pages makes the primitive gallery clickable with no server to run. Verify: capture shots from a real local run (live-verify style); Pages deploy green + gallery loads in both themes. Plan → sign-off. |
