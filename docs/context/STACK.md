...
| next | ^16.3.3 | App Router only; Pages Router is legacy. 16.2.11 patched the 2026-07-22 advisory batch (9 GHSAs); 16.2.12 backports TS7 handling (see the `typescript` row); **16.3.3 taken 2026-08-26** (August security release — AVIF-decode RCE via `sharp`/libheif, reachable through the upload surface; route-(2) age-gate exception, the `sharp` override retired with it). 16.3.1 was taken and reverted 08-22 on a standalone-only boot crash → Dependency-policy rule 6 in MAINTENANCE.md. Row corrected 2026-09-01 (it had sat at `^16.2.12` through both takes) |
...
| @next/eslint-plugin-next | ^16.2.12 | Tracks Next.js major; meant to ride the same patch train as `next` (it publishes in lockstep, so a stale lockfile pin is invisible drift — `manypkg` can't see it). **Lagging since the 16.3.3 take (2026-08-26):** the lockfile resolves 16.2.12 while `next` is 16.3.3 — deliberately left outside the security exclude; bump it in lockstep at the 16.3.4 take (its own `pnpm add` in `tooling/eslint`) |
...
| vitest | ^4.1.9 | verified 2026-06-23; eight workspaces carry a `vitest.config.ts` (`apps/web`, `validators`, `ui` (jsdom), `db`, `auth`, `calendar`, `email`, `jobs`). **v4 uses the built-in oxc transformer** (the automatic JSX runtime works with no `@vitejs/plugin-react`); a leftover `esbuild.jsx` config is ignored with a warning |
