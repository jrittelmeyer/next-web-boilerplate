...
## Versions (rows last verified 2026-08-02)
...
| next | ^16.3.6 | App Router only; Pages Router is legacy. 16.2.11 patched the 2026-07-22 advisory batch (9 GHSAs); 16.2.12 backports TS7 handling (see the `typescript` row); **16.3.3 taken 2026-08-26** (August security release — AVIF-decode RCE via `sharp`/libheif, reachable through the upload surface; route-(2) age-gate exception, the `sharp` override retired with it — a new `sharp: 0.35.4` override followed 2026-09-08 for the libheif HIGHs, since next's own pin excluded that fix too); **16.3.6 taken 2026-09-23** (GHSA-vcvr-r3jv-pc5j, RCE in `next/og`'s `ImageResponse` — live on three public image routes; route-(2) exception, ten dated `minimumReleaseAgeExclude` entries until 2026-09-29; incorporates 16.3.4/16.3.5). 16.3.1 was taken and reverted 08-22 on a standalone-only boot crash → Dependency-policy rule 6 in MAINTENANCE.md. Row corrected 2026-09-01 (it had sat at `^16.2.12` through both takes) and again 2026-09-23 |
...
| typescript | ^6.0.3 | **Stay on TS 6.** TS 7 GA'd (`7.0.2`, 2026-07-08) as the native Go compiler but **dropped the JS Compiler API** (`ts.createProgram`/`readConfigFile` etc.); the programmatic API returns in **TS 7.1 (~Q4 2026)**. The `tsc` CLI works and is ~3.6× faster; a 2026-07-13 cutover attempt was reverted. The Next-side gate lifted 2026-08-02 (`experimental.useTypeScriptCli` in stable `next@16.2.12`) — **but that is no longer the binding constraint.** ⚠️ **TS 7 ships no `tsserver`** (`bin` is `{tsc}`; TS 6's is `{tsc, tsserver}`), so bumping the workspace TypeScript leaves the editor's "Use Workspace Version" with no server and the `next` **tsserver plugin** (`tooling/typescript/nextjs.json`) with no host — the build and the editor would check with different compilers, and **no CI lane can observe it**. TS 7 is also a native binary published for 20 platform tuples with **no musl variant**, which a generated project inherits (the Docker builder is `node:24-alpine`). Re-gated accordingly. See [BACKLOG.md](../BACKLOG.md) |
...
| @next/eslint-plugin-next | ^16.2.12 | Tracks Next.js major; meant to ride the same patch train as `next` (it publishes in lockstep, so a stale lockfile pin is invisible drift — `manypkg` can't see it). **Lagging since the 16.3.3 take (2026-08-26), still after 16.3.6 (2026-09-23):** the lockfile resolves 16.2.12 while `next` is 16.3.6 — deliberately left outside both security excludes; bump it in lockstep with the next routine `next` take (its own `pnpm add` in `tooling/eslint`) — tracked in MAINTENANCE.md's 16.3.6 Watch bullet |
...
| @t3-oss/env-nextjs | ^0.13.11 | typed env validation (`apps/web/src/env.ts`); range recorded 2026-09-01 — the row had said "latest — verify at install" since the scaffold |
| vitest | ^4.1.11 | verified 2026-06-23; floor raised 2026-09-08 for GHSA-82fw-gwwq-j7x9 (`@vitest/mocker` path traversal; `vitest@5` exists but is a new major); eight workspaces carry a `vitest.config.ts` (`apps/web`, `validators`, `ui` (jsdom), `db`, `auth`, `calendar`, `email`, `jobs`). **v4 uses the built-in oxc transformer** (the automatic JSX runtime works with no `@vitejs/plugin-react`); a leftover `esbuild.jsx` config is ignored with a warning |
| @vitest/coverage-v8 | ^4.1.11 | verified 2026-06-23; V8 coverage provider, must match `vitest` exactly (lockstep peer). Not in the default `test` task — run `pnpm --filter <pkg> exec vitest run --coverage` |
...
| vite | 8.0.16 (override + exact devDep) | verified 2026-06-23; pulled **transitively** by vitest (dep range `^6\|\|^7\|\|^8`) and declared **directly** as an exact devDep in `packages/ui` for Storybook's Vite builder. Never imported by app code; the `pnpm-workspace.yaml` `overrides` entry pins every transitive copy to the same aged release — bump both together as newer 8.x ages out |
...
| posthog-js | 1.391.2 | verified 2026-06-23; client SDK in `apps/web` (the `PostHogProvider` mounted in `app/[locale]/layout.tsx`). **Exact pin.** Pulls `core-js` (build script left `false` — postinstall is only a funding banner). `posthog-js/react` provides `PostHogProvider` |
...
| knip | 6.35.1 (exact) | root devDep (2026-07-12) — dead-code / unused-dep gate (`pnpm knip`, CI `verify` lane; config `knip.jsonc`). **Exact pin** (publishes near-daily; the newest aged release is pinned — bump as newer releases age out). 6.24.0 → 6.35.1 on 2026-09-22 closed the `smol-toml` HIGH (GHSA-7w5x-hrqm-74c2) without an override; the newer resolver surfaced two pre-existing findings (see CHANGELOG) |
