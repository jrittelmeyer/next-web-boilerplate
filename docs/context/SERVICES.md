...
- Import rule: `@repo/email` may import from `@repo/validators` and `@repo/db` (the
  suppression consult, #8 — see ARCHITECTURE.md). It is consumed by `@repo/auth`
  (the Better Auth callbacks) and app Server Actions.

**Structure:**
```text
packages/email/src/
  client.ts                   — import "server-only" + lazy getResend() (guarded singleton)
  send.tsx                    — isEmailConfigured() + sendVerificationEmail / sendPasswordResetEmail / sendWelcomeEmail / sendChangeEmailConfirmationEmail / sendNewEmailVerificationEmail / sendEmailChangedNoticeEmail / sendDeleteAccountVerificationEmail / sendOrganizationInvitationEmail / sendMagicLinkEmail — plus the suppression consult (#8, gated on RESEND_WEBHOOK_SECRET; see below) and the TEST-ONLY EMAIL_TEST_CAPTURE_DIR seam (see TESTING.md → Email capture)
...
- `RESEND_WEBHOOK_SECRET` — the svix signing secret (`whsec_…`) for
  `/api/resend/webhook`; also arms the send helper's suppression consult (#8, next
...
- **Send-side gate:** the consult runs only when `RESEND_WEBHOOK_SECRET` is set (unset
  ...
  behave byte-identically to pre-#8). It **fails open** with a logged warning if the
...
callback ([→ Uploadthing](#uploadthing-file-uploads)): point the Resend webhook
...
3. **Fully remove `@repo/email`:** only after neutralizing every caller — the Better Auth email
   ...
   job, and the `/api/resend/webhook` route (+ its `email_suppressions` table, #8) — and
...
**Local collector recipe** (how #9 was verified; also the quickest way to *see*
traces):
