...
> **Status:** Phases 0–1. The time core (`civil.ts`, `timezone.ts`, `derive.ts`) and the
> `calendars` / `calendar_events` schema exist. `recurrence_parent_id`, `recurrence_id`,
> `rrule` and `series_end_at` are **declared, indexed and constrained but inert** —
> nothing writes them until Phase 2. Recurrence, occurrences, ICS and free/busy land in
