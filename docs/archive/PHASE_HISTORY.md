...
| Tier 4 · B2 (audit) | Persisted `audit_log` (migration 0011) + shared best-effort `recordAuditEvent()`; FK-less actor/target ids. 2026-07-09. See [context/AUTH.md](../context/auth/rbac-admin.md#persisted-audit-trail--audit_log-b2). |
| Tier 4 · B2 (audit UI) | `/admin/audit` read surface — keyset-paginated, LEFT-JOIN-resolved emails, uuid-cursor guard. 2026-07-09. See [context/AUTH.md](../context/auth/rbac-admin.md#persisted-audit-trail--audit_log-b2). |
...
| Tier 4 · B3 (rate-limit storage) | Better Auth limiter → `rateLimit.storage: "database"` (`rate_limit` table, migration 0013; atomic check-and-increment). 2026-07-10. See [context/AUTH.md](../context/auth/core.md#multi-instance-storage). |
...
- **INLINE, not modal — a deliberate UX call (the original blocker is now fixed).** Both the
  ...
  overflow-y-auto` (see [UI.md](../context/UI.md) → Dialog + the `TallContent` story in
...
- **`typedRoutes` evaluated and NOT adopted (Tier 4 · A31, 2026-07-12).** The stable top-level
  ...
  - **The app's real link surface is out of `typedRoutes`' reach.** ~All links/redirects flow
    through `@/i18n/navigation` ([I18N.md](../context/I18N.md)), and next-intl's `createNavigation` types are
...
## Archived 2026-09-02: PROJECT_STATUS history and BACKLOG's audit-pass narrative

_Moved verbatim from `docs/PROJECT_STATUS.md` / `docs/BACKLOG.md` on 2026-09-02 (doc audit); only relative link paths were rewritten for this location. The compact residues in the source files point here._

### PROJECT_STATUS `_Last updated_` line — as it stood 2026-09-02
...
### PROJECT_STATUS "Where we are" history bullets — as it stood 2026-09-02
...
- **Phases 1–2 complete & verified** — full-stack breadth (Steps 1–16) hardened to the
  ...
  correctness bugs** ([archive/PHASE_B_AUDIT.md](PHASE_B_AUDIT.md)).
- **Phase 3 (feature depth) + the 100/100 audit backlog complete & on `main`** — Tier 0 ·
  ...
  [archive/PHASE_HISTORY.md](PHASE_HISTORY.md)).
...
- **Every locally-buildable Tier-4 row SHIPPED (2026-07-07 → 13)**, incl. A23–A32 + A13.
  ...
  (latest: [PROJECT_AUDIT_2026-09-01.md](PROJECT_AUDIT_2026-09-01.md) — the
...
- **The path-to-100 program (owner decision, 2026-07-15) is BUILD-COMPLETE and VERIFIED
  ...
  ([per-row analysis](PATH_TO_100_2026-07-15.md) ·
  [the verifying pass](PROJECT_AUDIT_2026-07-17.md)). **Maintenance-only is the
...
### PROJECT_STATUS ai-dev-kit bullet — as it stood 2026-09-02
...
- **ai-dev-kit:** the repo's agentic-dev techniques are a portable skill library — the
  ...
  the kit repo's CHANGELOG + [archive/PHASE_HISTORY.md](PHASE_HISTORY.md).
### BACKLOG Tier-4 audit-pass narrative (blockquote) — as it stood 2026-09-02
...
> [archive/PATH_TO_100_2026-07-15.md](PATH_TO_100_2026-07-15.md)) — **shipped all
> 11 build rows 2026-07-16 → 17** (rows archived →
> [archive/PHASE_HISTORY.md](PHASE_HISTORY.md)), closed the last remainder
...
> ([archive/PROJECT_AUDIT_2026-07-22.md](PROJECT_AUDIT_2026-07-22.md)). The
...
> ([archive/PROJECT_AUDIT_2026-07-29.md](PROJECT_AUDIT_2026-07-29.md)). The
...
> ([archive/PROJECT_AUDIT_2026-08-04.md](PROJECT_AUDIT_2026-08-04.md)). The
...
> ([archive/PROJECT_AUDIT_2026-08-06.md](PROJECT_AUDIT_2026-08-06.md)). The
...
> 2026-08-14 ([archive/PROJECT_AUDIT_2026-08-13.md](PROJECT_AUDIT_2026-08-13.md)).
...
> ([archive/PROJECT_AUDIT_2026-08-19.md](PROJECT_AUDIT_2026-08-19.md)). The
...
> ([archive/PROJECT_AUDIT_2026-09-01.md](PROJECT_AUDIT_2026-09-01.md)).

## Archived 2026-09-23: ninth compaction (PROJECT_STATUS rows 2026-08-02 to 09-08) and the BACKLOG shipped-row sweep

_Moved verbatim from `docs/PROJECT_STATUS.md` / `docs/BACKLOG.md` on 2026-09-23 (doc audit); only relative link paths were rewritten for this location. PROJECT_STATUS keeps five cluster rows (one per program window) pointing here; BACKLOG's Shipped table keeps the rows dated 2026-09-02 and later._

### PROJECT_STATUS per-program rows, 2026-08-02 to 2026-09-08 (58 rows, verbatim)
...
| Audit — 2026-08-04 | 1 | Thirteenth pass, first to score the calendar: **98.6**; calendar enters at 85 — F4 external-guest cancellations (HIGH), F5 series-delete footgun, F6 verified-email seams, F7/F8 silent recurrence edges. 14 rows → BACKLOG; drift fixed in 6 docs | [report](PROJECT_AUDIT_2026-08-04.md) |
...
| Calendar F4+F5 — 2026-08-05 | 1 | F4: `ne(userId, actor)` NULL-dropped externals from cancellations — fixed `or(isNull, ne)`; real-PG test proves both spellings, the mocked fixture had asserted what SQL contradicted. F5: delete now runs `scopePairIssues`; first scope-pair tests | [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
...
| Calendar F7+F8 — 2026-08-06 | 1 | F7: the `overlaps` seek reaches back a full occurrence span — straddlers ≥2 periods out never generated. F8: `YEARLY;BYMONTHDAY` sans `BYMONTH` expands every month (corpus +40 append-only; COUNT/BYSETPOS rows re-identify — census-gated: zero) | [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
...
| Audit — 2026-08-06 | 1 | Fourteenth pass: **99.3** (calendar 85→95.5). F4–F8 adversarially re-verified at their seams; live surface fully green (heartbeat + security lanes same day). New find: the F4/F6-respond fixes lack revert sensors — B2 sweep row sharpened, now first in band | [report](PROJECT_AUDIT_2026-08-06.md) |
...
| Audit — 2026-08-13 | 1 | Fifteenth pass: **99.3** flat — 08-08 closures verified (API 100, sensors red-proven by reconstruction); found `main` red (un-gated kit-0.13.0 push fails Biome — B1 row), nanoid advisory re-widened (dated take 08-14), bare brace-expansion key (rider) | [report](PROJECT_AUDIT_2026-08-13.md) |
...
| Audit — 2026-08-19 | 1 | Sixteenth pass: **99.4** — 08-14 executions verified (F5 closed → Security 100; `main` green → Testing 99); checkpoint hook reviewed clean (anti-F2 process); found the better-auth watch gap (breaking 1.7 untracked) → dated take added; zero new rows | [report](PROJECT_AUDIT_2026-08-19.md) |
...
| Doc audit — 2026-08-31 | 1 | Renovate reality reconciled across STATUS/BACKLOG/MAINTENANCE/DEPLOYMENT; v1.2.0 row struck; Docker follow-up closed; archive index +2 (its rule slipped a 3rd time); kit 0.23.11 currency; `next` 16.3.4 dated; showcase re-stamped; 9 memory repairs | [archive/README.md](README.md) |
| Harness audit — 2026-08-31 | 1 | First agent-harness pass (kit 0.23.11 · CC 2.1.251): 16 authorities re-fetched, with/without eval sample (live-verify 3/3 skill-only), **93.9** baseline; template allowlist `winget`/`docker exec` grants, legacy `prodVerify` adapter, 0.23.13 `disable-model-invocation` ↔ Stop-hook clash; 4 rows, contrarian-folded | [report](HARNESS_AUDIT_2026-08-31.md) |
| Audit — 2026-09-01 | 1 | Seventeenth pass: **99.3** — 08-26 takes, CVE fix, v1.2.0 verified; new: fork-red `renovate.yml` (B1), month-boundary e2e red tracked (B2), draft releases (B3), STACK.md stale after both takes (fixed); 14 drift fixes; contrarian 13/13 folded | [report](PROJECT_AUDIT_2026-09-01.md) |
...
| Renovate B1 — 2026-09-03 | 1 | Host decision closed: Mend kept (scoped to the no-lockfile class, #56 merged); `renovate.yml` kept dormant as cold fallback, `ENABLE_RENOVATE` unset; npm-manager delivery tracked as an accepted open risk | [host decision plan](renovate-b1-host-decision-plan.md) |
...
| Doc audit — 2026-09-02 | 1 | ~85 drift fixes across 27 docs (five verifier clusters, ~1,900 claims); MAINTENANCE's landed Watch → new `archive/WATCH_HISTORY.md` (78K → 51K chars); STATUS/BACKLOG compacted; showcase at 17 passes/99.3; 12 memory repairs; contrarian 13/13 folded | [archive/WATCH_HISTORY.md](WATCH_HISTORY.md) |
...
### BACKLOG shipped rows swept 2026-09-23 (13 rows, verbatim)
...
| B1 | ~~F4 — external guests never received cancellation emails (`or(isNull, ne)` + real-PG recipient-set tests)~~ | 2026-08-05 | [CHANGELOG](../../CHANGELOG.md) · [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
| B1 | ~~F5 — `deleteEventSchema` refuses the scope/recurrenceId half-pair (shared `scopePairIssues`; first scope-pair tests)~~ | 2026-08-05 | [CHANGELOG](../../CHANGELOG.md) · [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
...
| B2 | ~~F7 — the `overlaps` seek reaches back a full occurrence span (opt-in `seekBackDays` = per-master span + zone slack; completeness tests, span > 2 periods)~~ | 2026-08-06 | [CHANGELOG](../../CHANGELOG.md) · [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
| B2 | ~~F8 — `YEARLY;BYMONTHDAY` without `BYMONTH` expands every month; corpus samples the unpaired family (+40 → 568, SHA repinned, census zero)~~ | 2026-08-06 | [CHANGELOG](../../CHANGELOG.md) · [audit 08-04](PROJECT_AUDIT_2026-08-04.md) |
