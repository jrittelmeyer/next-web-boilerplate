...
_Last updated: 2026-09-23 — doc audit: the three post-09-22 commits reconciled (the `smol-toml` red closed via `knip` 6.35.1 and #61 auto-closed; `next` 16.3.6 taken as an RCE exception with ten age-exclude entries until 09-29; the B3 calendar long-tail batch signed and built); ninth compaction of the table below (58 rows → archive); STACK.md's `next`/`knip`/`vitest` rows, the invitations token-expiry claim and the archive index caught up; a "cut v1.3.0" B3 row filed ([BACKLOG](BACKLOG.md))._
