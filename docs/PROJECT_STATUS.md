...
- The committed `.claude/` directory holds three layers with different owners: the
  ...
  stays untracked (gitignored). Ownership rules, why the repo-owned handler sits outside
  the kit directory, and **why a valid agent file may still not register** (it is
  surface-dependent, not fixed by reloading):
