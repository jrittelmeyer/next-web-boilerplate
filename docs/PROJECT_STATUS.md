...
- The committed `.claude/` directory holds three layers with different owners: the
  ...
  2026-07-27); and `settings.json`, which is user-owned and merged rather than
  regenerated by the installer. `settings.local.json` stays untracked (gitignored).
  Ownership rules + why the repo-owned handler sits outside the kit directory:
