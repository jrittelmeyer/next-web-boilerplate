...
[← The Calendar](12-the-calendar.md) · [Guide index](README.md)
...
**Civil time / instant** — A wall-clock reading with no timezone ("09:00 next Monday") / an actual point on the timeline. Never interchangeable; treating them as one is the source of most calendar bugs. [Ch. 12](12-the-calendar.md)
...
**Daylight-saving transition** — The twice-yearly clock change. It makes one wall-clock reading never happen and another happen twice, and its size is not always an hour (Lord Howe Island shifts by 30 minutes). [Ch. 12](12-the-calendar.md)
...
**iCalendar / `.ics`** — The universal calendar-file standard every mail and calendar program understands; the attachment that turns an invitation into an entry in one click. [Ch. 12](12-the-calendar.md)

**Idempotent (fire-exactly-once)** — An operation safe to attempt repeatedly because repeats have no additional effect — how two servers racing to send the same reminder still send it once. [Ch. 12](12-the-calendar.md)
...
**Occurrence / override** — One dated instance generated by a repeating event / the stored exception recording that a single occurrence was moved or changed. [Ch. 12](12-the-calendar.md)
...
**Recurrence rule (`RRULE`)** — "Every second Tuesday" stored as a rule rather than as thousands of individual entries, and unrolled only as far as the screen being drawn. [Ch. 12](12-the-calendar.md)
...
**RSVP / attendee** — A guest on an event and their answer (accepted, declined, tentative, or not yet answered). Here the guest is identified by email address, so an invitee needs no account. [Ch. 12](12-the-calendar.md)
