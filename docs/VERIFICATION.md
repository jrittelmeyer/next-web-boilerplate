...
>
> 🔒 **Account-takeover fix re-verified 2026-07-27** (GHSA-qq9h-g4jm-xgf3, `better-auth`
> 1.6.23) by driving the actual attack against a fresh prod build with email configured,
> not by reading the changelog: an attacker signs up the victim's address with their own
> password (address unverified) → the victim requests and consumes a real magic link
> (captured send is `/api/auth/magic-link/verify`) → session established,
> `emailVerified: true` → the attacker's password now returns **401
> `INVALID_EMAIL_OR_PASSWORD`**. Pre-fix that final step returns 200 on a verified
> account, which is the takeover. The unproven credential is revoked, per 1.6.22.
> **Behavior change to expect:** a password set but never verified stops working once its
> owner signs in passwordlessly.
>
